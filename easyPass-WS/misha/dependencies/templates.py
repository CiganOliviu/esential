...
class init_templates():
    ...
    def standard_structure_cpp_algo_data_structures_templates(self, file_object):

        file_object.write("#include \"dataStructures.hpp\" \n\n")
        file_object.write("int main(int argc, char const *argv[]) {\n\n")
        file_object.write("\tOneDimensionalArrayType<int> ODAObject;\n")
        file_object.write("\tmatrixType<int> MTObject;\n")
        file_object.write("\tbinaryTreeType<int> binaryTreeObject;\n\n")
...
class templates(init_templates):
    ...
    def standard_cpp_algo_data_structures_templates(self, file_name):

        ADS_template = open(file_name, "w")

        self.standard_structure_cpp_algo_data_structures_templates(ADS_template)

        ADS_template.close()

    def standard_c_algo_data_structures_templates(self, file_name):

        ADS_template = open(file_name, "w")

        self.standard_structure_c_algo_data_structures_templates(ADS_template)
